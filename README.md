...
修正部分错误的文本，删去一些多余的空格  
-1.03新增（2025/09/10）：  
修正了部分书籍内容文本没有正常汉化的问题，修正了装备文件传错的问题。删去一些多余的空格。  
-1.04新增（2025/10/03）：  
修正部分多余符号  
修复了之前的汉化导致金丝桃酊无法正常使用和发挥效果的问题  
-1.05新增（2025/12/26）：  
修复了某个事件的名称被误改为空字符的问题等  
...
-1.04新增（2025/10/03）：  
修正了在已经接取Robin的刀刃任务时，在Wind Tower开箱子会重复接取刀刃任务的问题  
修改了击败Baal之后的对话时机，以便达到在已经为Robin做过刀以后Robin不会再问起自己的刀的效果  
-1.05新增（2025/12/26）：  
修复了之前修Halaina曲奇店bug导致的会在选择非正确物品时额外将选择的物品（若取消则会是空物品）加入背包的问题。已经触发该bug导致背包无法打开的玩家请到补丁下载地址（https://github.com/kazeever/Aveyond4-zh_CN-translationpatch）下载补丁 存档修复（背包空物品），并将其置入游戏文件夹（Aveyond 4 Shadow Of The Mist）覆盖所有文件，并在Halaina曲奇店内（请注意需要切换地图才能刷新旧地图的事件）和新增的npc互动即可。尚未触发此bug的玩家无需下载该存档修复补丁。  
...
-在不明情况下，打开物品栏或者进行任何需要选择物品的操作会导致闪退，触发条件和原因、解决方法均不明。该bug发生率并不高，因此暂时不确定是什么情况，请多备份存档或者经常检查背包（并非必须，在汉化过程中的全部测试内仅触发过一次该问题）。如果遇到此类bug请参考后述的联系方式发送存档以便研究bug机制。  
以下为非bug的特性。  
-出于系统上的原因，作用对象为全体的所有物品（目前具体来说也就是金丝桃酊和一轮芝士）均不能在菜单中使用，它们只能在战斗中使用。  
-在流程设计上，游戏中只能在Robin入队以后的剧情里才可以买到sid谜题里的某些物品。但是，由于玩家可能在Goodie Cave（在前几部的汉化版内译为糖果洞窟）里提前获得对应物品，可能会导致在Robin入队之前就解开全部谜题，并因此触发Robin的对话。  
